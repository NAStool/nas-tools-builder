...
RUN set -xe && \
    ...
    rm -rf \
        ...
        /var/lib/apt/lists/* \
        /var/tmp/*
RUN ln -sf /command/with-contenv /usr/bin/with-contenv
...
RUN set -xe \
    ...
    && cp -r ${WORKDIR}/docker/rootfs/* / \
    && chmod +x -R /etc/cont-init.d /etc/services.d
